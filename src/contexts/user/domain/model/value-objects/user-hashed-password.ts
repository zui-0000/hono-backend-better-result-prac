 ...
 *
 * `z.hash()` は使えない。あれが見るのは**ハッシュ値そのもの** (sha256 なら 64 桁の
 * 16 進) で、ここが持つのはパラメータとソルトを同梱した PHC 文字列。実測で
 * argon2id の値が弾かれた。アルゴリズムを名指しする API なのも、上記のとおり
 * 名指ししたくないという狙いと逆になる。
 *
 * なお契約 (`schema/`) にこの型は無い。ハッシュは API に出ないため、
 * 他の値オブジェクトと違って**契約との突き合わせは不要**。
 */
export const UserHashedPassword = z
  .string()
  .regex(PHC_PATTERN)
  .brand<"User.HashedPassword">();
export type UserHashedPassword = z.infer<typeof UserHashedPassword>;
