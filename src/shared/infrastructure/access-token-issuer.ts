...
/**
 * アクセストークンの寿命 (秒)。**この値がそのまま「取り消しが効くまでの最大遅延」になる。**
 * 券は DB を引かずに検証するので、ログアウトも失効も発行済みの券には届かない。
 *
 * 1 時間は Google / Okta / Cognito の既定値と同じ。短くすれば失効が速くなるが、
 * 更新の回数 (＝ DB アクセス) が増える。
 */
 ...
 */
export const accessTokenIssuer = (secret: string): AccessTokenIssuer => {
