 ...
 * 実装を結線してよいのは合成ルート (src/app-deps.ts) だけ。
 */
const PORT_SIDE =
    ...
    },
    {
      ...
      comment: message({
        ...
        fix:
          ...
          "必要な依存は handleWithResult が受け取るランタイム経由で解決され、\n" +
          "実装の結線は src/app-deps.ts が行います。",
      }),
    ...
    },
    {
      ...
      comment: message({
        ...
        fix:
          ...
          "例: コンテキストが要求するサービスの型は、実装から導出せず\n" +
          "ポートを列挙して組み立てます (contexts/user/user-deps.ts の UserDeps)。",
      }),
      from: { path: PORT_SIDE },
      to: { path: IMPL_LAYER, reachable: true },
    },
    // DB 専用のルールは置かない。Drizzle まわりは shared/infrastructure/db/ に
    // あって IMPL_LAYER に含まれるため、上の 4 ルール (とくに到達可能性で見る
    // no-indirect-path-to-impl) が同じことを覆う。
    {
