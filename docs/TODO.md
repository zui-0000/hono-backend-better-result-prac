...
（[`CLAUDE.md`](../CLAUDE.md) の「依存の渡し方」）。もっと楽に書ける道があるかを調べる。
...
**このリポジトリの前身は Effect をコアに据えて組んでいた。** やめた理由は
機能の不足ではなく、**統一のコスト**。

Effect はライブラリではなく**ランタイムと生態系**で、入れると全部が Effect に
なりたがる（エラー / DI / 検証 / HTTP / 並行 / 資源）。**全部乗りにしないと
元が取れない**設計なので、半分だけ乗せるのが一番割に合わない。

いまの構成は **Effect の「形」だけ取ってランタイムを捨てたもの**。

| 取ったもの         | いまの実装                 |
| ------------------ | -------------------------- |
| 短絡する制御フロー | `Result.gen` + `yield*`    |
| 網羅的なエラー処理 | `TaggedError` + `.match()` |
| 差し替え可能な依存 | ポート + 部分適用          |

捨てたのは `Layer` / `Scope` / `Fiber` / 割り込み。**いまの規模では要らない**
（11 ポート・DB プールは 1 個・ハンドラは互いに独立・長時間処理が無い）。

**したがって `Layer` は DI の選択肢に入れない。** DI のためだけにランタイムごと
再導入するのは本末転倒で、それは「Effect に全部乗せるか」という別の判断になる。

### 調べること

**「どのライブラリか」ではなく「ライブラリが object literal より何を足すか」。**

TS の DI コンテナはほぼ**クラスとデコレータ前提**（tsyringe / InversifyJS /
TypeDI / diod）で、`reflect-metadata` を要求するものも多い。あれが解いているのは
**OO のコンストラクタ注入が手配線だと冗長で順序依存になる**問題であって、
関数とレコードで書く側にはその問題が無い。**関数型 TS における「コンテナ」は
オブジェクトリテラルそのもの**で、`app-deps.ts` が既にそれ。

|          | 見るもの                        | 判断の観点                                                    |
| -------- | ------------------------------- | ------------------------------------------------------------- |
| **現状** | 部分適用 + 合成ルート           | 何が不足しているのかを先に 1 行で言う                         |
| 1        | Awilix                          | 文字列キー / プロキシ解決。**型で追えなくなる**方向           |
| 2        | tsyringe / InversifyJS / TypeDI | デコレータと OO 前提。関数型の書き方と噛み合わない            |
| 3        | Reader / `ReaderTaskEither`     | 依存をモナドで運ぶ。`Result.gen` の書き味と両立するかは要検証 |
| 4        | 小さな自作ヘルパ                | 「要求より広い束」だけを狭める薄い型ユーティリティで足りるか  |

**ライブラリが効きはじめる合図**（いまはどれも来ていない）:

- リクエストスコープの依存が要る（現状は全部シングルトン）
- **資源の取得と解放が対で要る**（`Scope` / `acquireRelease` が解く領域）
- 依存が増えて合成ルートが一望できなくなる

### 着手の引き金

**上の「合図」がどれか来たとき**、またはコンテキストが 3 つ目になったとき。
調べるなら時間を区切って、`deleteUserCommand` 1 本を現状 / Reader / 案 4 の
3 通りで書き比べ、**行数・読みやすさ・ポートを 1 本足すときの変更範囲**を測る。
差が出なければ「現状で足りている」という答えが出る。
